 ...
 * Vercel 版：1:1 移植自原项目（倪海夏《天纪》体系：四化永远固定）
 */

// @ts-nocheck -- iztro 返回的 astrolabe 类型推断不友好（map callbacks 隐式 any），
// 整文件类型检查关闭，因为 iztro → Palace[] 的转换本质是 schema mapping，不是类型推导问题。
// 类型安全由 ZiweiChart 输出端保证（lib/ziwei/types.ts 严格）。
